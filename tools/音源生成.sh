...
if [[ "${1:-}" != "--foley-only" ]]; then
  tools/audio/render.sh "$@"
  bash tools/音源生成.sh --foley-only
  exit 0
fi
...
amb() {
  ...
  ffmpeg -hide_banner -loglevel error -y -f lavfi -i "$graph" -t 60 \
    -af "aformat=channel_layouts=stereo,lowpass=f=6000,afade=t=in:st=0:d=2,afade=t=out:st=57:d=3,loudnorm=I=-28:TP=-8:LRA=7" \
    -ar 48000 -c:a libvorbis -b:a 128k "assets/audio/${id}.ogg"
}
se() {
  local id="$1" duration="$2" graph="$3"
  ffmpeg -hide_banner -loglevel error -y -f lavfi -i "$graph" -t "$duration" \
    -af "aformat=channel_layouts=stereo,lowpass=f=6500,afade=t=out:st=$(awk "BEGIN { print $duration - 0.08 }"):d=0.08,loudnorm=I=-22:TP=-7:LRA=7" \
    -ar 48000 -c:a libvorbis -b:a 96k "assets/audio/${id}.ogg"
}
...
amb amb_library "sine=f=440:r=44100,volume='if(lt(mod(t,7.3),0.055),0.025*exp(-38*mod(t,7.3)),0)':eval=frame[a];anoisesrc=c=pink:r=44100:seed=804,highpass=f=500,lowpass=f=2600,volume=0.010[b];[a][b]amix=inputs=2:normalize=0"
...
amb amb_kitchen "anoisesrc=c=pink:r=44100:seed=807,highpass=f=600,lowpass=f=3600,volume='if(lt(mod(t,6.7),0.09),0.11*exp(-32*mod(t,6.7)),0)':eval=frame"
...
amb amb_hidden_room "sine=f=420:r=44100,volume='if(lt(mod(t,5.1),0.06),0.07*exp(-45*mod(t,5.1)),0)':eval=frame[a];anoisesrc=c=pink:r=44100:seed=809,highpass=f=500,lowpass=f=2800,volume='if(lt(mod(t,5.1),0.13),0.032*exp(-24*mod(t,5.1)),0)':eval=frame[b];[a][b]amix=inputs=2:normalize=0"
...
se se_glass 1.10 "sine=f=720:r=44100,volume='0.14*exp(-7*t)':eval=frame[s0];sine=f=1080:r=44100,volume='0.07*exp(-9*t)':eval=frame[s1];anoisesrc=c=pink:r=44100:seed=701,highpass=f=450,lowpass=f=3200,volume='0.045*exp(-16*t)':eval=frame[s2];[s0][s1][s2]amix=inputs=3:normalize=0"
...
se se_clock 0.75 "sine=f=520:r=44100,volume='0.14*exp(-9*t)':eval=frame[s0];sine=f=780:r=44100,volume='0.055*exp(-11*t)':eval=frame[s1];[s0][s1]amix=inputs=2:normalize=0"
...
se se_typewriter 0.45 "anoisesrc=c=pink:r=44100:seed=712,highpass=f=550,lowpass=f=3500,volume='0.10*exp(-18*t)':eval=frame[a];sine=f=210:r=44100,volume='0.13*exp(-13*t)':eval=frame[b];[a][b]amix=inputs=2:normalize=0"
se se_temari 0.80 "sine=f=145:r=44100,volume='0.28*exp(-7*t)':eval=frame[a];sine=f=290:r=44100,volume='0.11*exp(-9*t)':eval=frame[b];anoisesrc=c=brown:r=44100:seed=713,lowpass=f=420,volume='0.14*exp(-13*t)':eval=frame[c];[a][b][c]amix=inputs=3:normalize=0"
se se_tray 1.20 "anoisesrc=c=pink:r=44100:seed=714,highpass=f=450,lowpass=f=3800,volume='0.10*exp(-7*t)':eval=frame[a];sine=f=360:r=44100,volume='0.15*exp(-5*t)':eval=frame[b];sine=f=610:r=44100,volume='0.065*exp(-8*t)':eval=frame[c];[a][b][c]amix=inputs=3:normalize=0"
...
se se_keys 0.85 "sine=f=760:r=44100,volume='0.11*exp(-10*t)':eval=frame[a];sine=f=1080:r=44100,volume='0.055*exp(-13*t)':eval=frame[b];anoisesrc=c=pink:r=44100:seed=719,highpass=f=500,lowpass=f=3000,volume='0.035*exp(-18*t)':eval=frame[c];[a][b][c]amix=inputs=3:normalize=0"
se se_pen 0.55 "anoisesrc=c=pink:r=44100:seed=716,highpass=f=900,lowpass=f=4200,volume='0.12*exp(-13*t)':eval=frame[a];sine=f=340:r=44100,volume='0.10*exp(-12*t)':eval=frame[b];[a][b]amix=inputs=2:normalize=0"
se se_cloth 0.95 "anoisesrc=c=pink:r=44100:seed=717,highpass=f=350,lowpass=f=2900,volume='0.15*exp(-3*t)':eval=frame"
