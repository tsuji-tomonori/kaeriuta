...
const WORDS = {
  ...
  care: ['助', '寄り添', '話', '謝', '告白', '自首', '蘭', '母', '心', '信', '待', '守'],
  dialogue: ['話', '聞', '答', '任せ', '座', '伝', '問', '返事', '呼'],
  harmful: ['転嫁', '売', '偽', '毒', '殺', '隠', '逃', '出さない', '黙'],
  safe: ['待', '戻', '任せ', '穏', 'そのまま', '聞き流', '閉じ'],
...
export function specialPartPolicy(strategy) {
  return ({ option, observation, index }) => {
    ...
    if (observation.part?.name === 'jointReasoning') {
      ...
      if (strategy === 'hide') add(is(['コナン', '任せ']) ? 12 : (phase ? 6 - phase : 0), is(['コナン', '任せ']) ? '共同推理は代理に任せて関与を薄める' : '共同推理は代理可能な外堀から確認する');
