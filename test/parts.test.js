...
test('律の遺品は二回だけ使え、良心を無限に踏み倒せない',()=>{assert.equal(RELIC_USE_LIMIT,2);assert.equal(canUseRelic(0),true);assert.equal(canUseRelic(1),true);assert.equal(canUseRelic(2),false);});
test('N11で計画書が足りない理由を次周の調査へ結びつける',()=>{const hint=missingEvidenceHint({id:'n11',requirements:[['three_plans','three_plans_card']]},[]);assert.match(hint,/第一〜二章/);assert.match(hint,/共犯者の部屋/);assert.match(hint,/指示メモ/);});
test('証拠も代理も遺品もない主張は、明示的な未完導線の対象になる',()=>{assert.equal(hasReasoningMethod({id:'n11',requirements:[['three_plans']]},[]),false);assert.equal(hasReasoningMethod({id:'n1',requirements:[['record']],conanProxy:true},[]),true);});
test('A-4の結果表示をB-2と混同しない',()=>{assert.equal(resultTitle('a4_reversal'),'A-4 逆転への反証');assert.equal(resultTitle('b2_unfinished'),'B-2 未完END');});
test('通常推理を引き継がず、共同推理の種類ごとに専用BGMへ切り替える',()=>{assert.equal(jointReasoningBGM({variant:'a4'}),'bgm_joint_reasoning_a4');assert.equal(jointReasoningBGM({}),'bgm_joint_reasoning');});
